    ...
    useEffect(() => { //board sürekli güncelleniyor.
        ...
        const sweepRows = newStage => //dolmuş satır kontrol ve satır silme
            newStage.reduce((ack, row) => { //ack: accumulator, row: currentValue
                if (row.findIndex(cell => cell[0] === 0) === -1) { //satır full doluysa findIndex -1 döndürecek
                    ...
                    ack.unshift(Array.from({ length: newStage[0].length }, () => [0, 'clear'])); //yeni satır oluşturup en başa ekliyoruz.
                    return ack;
                }
                ack.push(row.map(cell => [cell[0], cell[1]])); //satır dolu değilse olduğu gibi ekliyoruz.
                return ack;
            }, []);
        ...
        const updateStage = prevStage => { //prevStage: mevcut stage duruu
            ...
            const newStage = prevStage.map(row =>
                row.map(cell => (cell[1] === 'clear' ? [0, 'clear'] : [cell[0], cell[1]])) //hareket eden taşı ilerletmek için önce stage temizleniyor. clear olanlar temizleniyor, merged olanlar kalıyor. 
            );
